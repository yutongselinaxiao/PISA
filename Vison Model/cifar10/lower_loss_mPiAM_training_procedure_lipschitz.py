...
def get_parser():
    ...
    parser.add_argument('--sigma_mode', type=str, default='fixed',
                        choices=['fixed', 'online_convex_bal',
...
def main():
    ...
    # ---- Adaptive-σ state (consumed in either OGD mode) ----
    floor = None
    last_floor_metrics = {}
    if args.sigma_mode in ('online_convex_bal_lipschitz', 'online_convex_bal'):
        use_floor = (args.sigma_mode == 'online_convex_bal_lipschitz')
        param_names = [n for n, _ in model.named_parameters()] if use_floor else None
